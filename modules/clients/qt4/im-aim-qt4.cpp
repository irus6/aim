...
class AimInputContext : public QInputContext
{
   Q_OBJECT
public:
   AimInputContext ();
...
AimInputContext::on_preedit_start (AimIM *im, gpointer user_data)
{
  ...
  AimInputContext *context = static_cast<AimInputContext *>(user_data);
...
QString
AimInputContext::identifierName ()
{
  g_debug (G_STRLOC ": %s", G_STRFUNC);
...
bool
AimInputContext::isComposing () const
{
  g_debug (G_STRLOC ": %s", G_STRFUNC);
...
void
AimInputContext::setFocusWidget (QWidget *w)
{
  g_debug (G_STRLOC ": %s", G_STRFUNC);
 ...
 */
class AimInputContextPlugin : public QInputContextPlugin
{
  Q_OBJECT
public:
  ...
  virtual QString description (const QString &key)
  {
    ...
    return QString ("aim Qt4 im module");
  }
};

Q_EXPORT_PLUGIN2 (AimInputContextPlugin, AimInputContextPlugin)

#include "im-aim-qt4.moc"

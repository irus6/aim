...
void
aim_send_message (GSocket        *socket,
                  AimMessageType  type,
                  gpointer        data,
                  guint16         data_len,
                  GDestroyNotify  data_destroy_func)
{
  ...
  AimMessage *message;
  const AimMessageHeader *header;
  GError *error = NULL;
  gssize n_written;

  message = aim_message_new_full (type, data, data_len, data_destroy_func);
  header  = aim_message_get_header (message);
  ...
  if (G_UNLIKELY (n_written < aim_message_get_header_size ()))
  {
    g_critical (G_STRLOC ": %s: sent %"G_GSSIZE_FORMAT" less than %d",
                G_STRFUNC, n_written, aim_message_get_header_size ());
    if (error)
    {
  ...
  /* debug message */
  const gchar *name = aim_message_get_name (message);
  if (name)
...
AimMessage *aim_recv_message (GSocket *socket)
{
  ...
  AimMessage *message = aim_message_new ();
  GError *error = NULL;
  gssize n_read = 0;
  ...
  if (G_UNLIKELY (n_read < aim_message_get_header_size ()))
  {
    g_critical (G_STRLOC ": %s: received %"G_GSSIZE_FORMAT" less than %d",
                G_STRFUNC, n_read, aim_message_get_header_size ());
...
void
aim_result_iteration_until (AimResult      *result,
                            ...
                            AimMessageType  type)
{
  ...
  if (G_UNLIKELY (result->is_dispatched == TRUE && result->reply == NULL))
    g_critical (G_STRLOC ": %s:Can't receive %s", G_STRFUNC,
                aim_message_get_name_by_type (type));
